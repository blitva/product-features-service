...
class ProductFeatures extends Component {
  ...
  getProductPhotos(productId) {
    return axios.get(`/photos/features/${productId}`)
  }

  fetchData(productId) {
    Promise.all([
      this.getProductFeatures(productId),
      // this.getProductPhotos(productId) // used for deployment
    ])
    .then((res) => {
      this.setState({
        productFeatures: res[0].data,
        // productPhotos: res[1].data.featuresUrls // used for deployment
      })
    })
    .catch((err) => {
      this.setState({
